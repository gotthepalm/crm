...
export default function CandidateForLinking({
	...
	return (
		<div
			className={`${isActive ? 'bg-violet-100 dark:bg-zinc-800' : 'bg-white dark:bg-zinc-900'} border-zinc-300 dark:border-zinc-700 component-transition h-full
			 w-full border rounded-2xl p-5 flex flex-col text-zinc-500 dark:text-zinc-400 text-[16px] font-medium cursor-pointer select-none`}
		>
			{/*Header*/}
			<div className='flex items-start text-black dark:text-white flex-col gap-2 border-b border-zinc-300 dark:border-zinc-700 mb-3 pb-3'>
				...
				{candidate?.vacancy?.position ? (
					<div
						className={`${isActive ? 'bg-violet-200 dark:bg-violet-900' : 'bg-violet-100 dark:bg-violet-950'} flex gap-3
					...
					candidate.position && (
						<div
							className={`${isActive ? 'bg-violet-200 dark:bg-violet-900' : 'bg-violet-100 dark:bg-violet-950'} component-transition
						 inline-flex items-center gap-2 font-medium px-4 py-1 rounded-xl`}
						>
							{candidate.position}
						</div>
					)
			...
			{candidate.meetings.length !== 0 && (
				<div className='flex flex-col items-start text-black dark:text-white gap-2 border-b border-zinc-300 dark:border-zinc-700 mb-3 pb-3'>
					{candidate.meetings.map((meeting) => (
						<div
							className={`${isActive ? 'bg-blue-200 dark:bg-blue-900' : 'bg-blue-100 dark:bg-blue-950'}
							 inline-flex items-center gap-2 border border-blue-300 dark:border-blue-500 
							 bg-blue-100 font-medium px-4 py-1 rounded-xl component-transition`}
							key={meeting.id}
						>
							<div
