...
import EditMeetingForm from '@/src/app/[locale]/crm/meetings/EditMeetingForm';
...
export default function Meeting({ meeting }: { meeting: MeetingModel }) {
	...
	return (
		<>
			...
			<article className='bg-white flex flex-col gap-3 rounded-2xl overflow-hidden border border-zinc-300 p-5 w-full h-full max-w-xl break-inside-avoid transition'>
				<div className='flex flex-col gap-3 items-start text-[18px] border-b pb-5'>
					...
					<span className='self-end text-[16px] font-medium px-3 py-1 rounded-xl bg-amber-100'>
						{t(`InterviewType.${meeting.interviewType}`)}
					</span>
				</div>
				{/*<div>Candidate and Vacancy</div>*/}
				{meeting.note && <p className='text-sm text-gray-600'>{meeting.note}</p>}
				{meeting.interviewers.length !== 0 && <div className='flex flex-col gap-2'>
