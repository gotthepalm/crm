...
import { getDbVacancyValues } from '@/src/app/[locale]/crm/vacancies/_actions/getDbVacancyValuesAction';
...
export default function EditVacancyForm({
	...
	const [vacancyValues, setVacancyValues] = useState<VacancyModel | null>(null);
	...
	useEffect(() => {
		getDbVacancyValues(vacancy.id).then((value) => setVacancyValues(value));
	}, []);
	return (
		<div className='backdrop-blur-sm bg-black/50 fixed inset-0 z-50 h-100dvh w-100dvw flex items-center justify-center'>
			<div className='max-w-[1600px] w-full h-[90%] mx-auto px-5'>
				<div className='h-full flex flex-col items-center overflow-y-scroll bg-white rounded-2xl px-30 py-10'>
					{vacancyValues ? (
						<>
